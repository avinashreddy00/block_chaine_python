...
new_df = df.tail(50)[['index', 'timestamp', 'data']]
...
if len(plot_data) > 0:
    ...
    plt.ylabel('Power in kilowatt hour')
...
def find_data(df, desired_hash, host):
    ...
    if not result.empty:
        time = result['timestamp'].iloc[0]
        data = result['data'].iloc[0]
        for entry in data:
            entry = eval(entry)
            if host == entry[0]:
                return time, entry[0], entry[1], entry[2]
    return None, None, None, None

# Call the function to get data
time_stamp, host_result, consumption_result, production_result = find_data(df, desired_hash, host)

# Display the result
if host_result is not None:
    st.write(f'Time: {time_stamp}')
